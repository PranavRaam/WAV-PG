...
const StageDetailView = ({ stage, onClose }) => {
  ...
  const [showPGSpecificMetrics, setShowPGSpecificMetrics] = useState(false);
  ...
  // Add a function to toggle between aggregate and PG-specific views
  const toggleMetricsView = () => {
    setShowPGSpecificMetrics(!showPGSpecificMetrics);
  };
  
  // Get current chart data based on selected time range and view mode
  const getCurrentChartData = () => {
    if (selectedPG && showPGSpecificMetrics) {
      // Return PG-specific metrics
      const pgId = selectedPG.id;
      const pgData = mockPGMetricsData[pgId] || defaultPGMetrics;
      return pgData[selectedTimeRange] || [];
    }
    
    // Return aggregate data (default)
    return mockKpiData[selectedTimeRange] || [];
  };
      ...
      {!selectedPG ? (
        ...
        <div className="pg-detail-view">
          ...
          <div className="pg-detail-grid">
            ...
            {/* Key Tactics Card */}
            <div className="detail-card tactics-card">
              <div className="detail-card-header">
                <div className="detail-card-title">
                  <span className="detail-icon">🎯</span>
                  Key Tactics
              ...
              <div className="tactics-stats">
                <div className="tactic-stat">
                  <div className="tactic-number">{getPGStats(selectedPG).totalTactics}</div>
                  <div className="tactic-label">Total Tactics</div>
                </div>
                <div className="tactic-stat">
                  <div className="tactic-number">{getPGStats(selectedPG).completedTactics}</div>
                  <div className="tactic-label">Completed</div>
                </div>
                <div className="tactic-stat">
                  <div className="tactic-number">{getPGStats(selectedPG).successRate}%</div>
                  <div className="tactic-label">Success Rate</div>
                </div>
              </div>
              ...
              <div className="tactics-list">
                {keyTactics[selectedPG.id].map((tactic, index) => (
                  <div key={index} className="tactic-item">
                    <div className={`tactic-status ${tactic.completed ? 'completed' : ''}`}>
                      {tactic.completed ? '✓' : '○'}
                    </div>
              ...
              <div className="tactics-dropdown">
                <div className="tactics-filter">All Tactics</div>
              </div>
            </div>
            ...
            <div className="detail-card pain-points-card">
              <div className="detail-card-header">
                ...
                <div className="communication-log">Communication Log</div>
              </div>
              ...
              <div className="pain-filter-dropdown">
